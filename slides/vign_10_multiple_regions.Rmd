...
subtitle: ""
author: "Emily Liljestrand and Tim Miller<br>NOAA Fisheries, NEFSC"
output:
 ...
 * Configuring a mean movement model
...
The `prepare_wham_input` function will create an input:
```{r, include = FALSE}
...
The `move` argument to `prepare_wham_input` or `set_move` has these elements to configure seasonal movement.

* `stock_move`: T/F whether each stock can move
* `separable`: T/F whether movement for each stock should be modeled separably from mortality or both occurring simultaneously
 ...
 * `"stock_constant"`: stock-speicfic movement rate to each region shared across all seasons, ages, years
 * `"stock_season"`: stock- and season-specific movement rate to each region shared across all ages, years
...
Because WHAM can treat movement and mortality as sequential or simultaneous, it tracks probability transition matrices that determine proportions at age being alive in different regions or dead in different fleets or due to M. So here there are 2 regions and 2 fleets in each region so there are 7 states to track:
* alive in each region (first two states)
* dead from catch in one of 4 fleets (next 4 states)
* dead due to natural mortality (last state)

Look at movement for northern stock at  age 8 each season (top left of P matrix)
```{r, eval = TRUE}
#first season
