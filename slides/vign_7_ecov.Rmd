...
* `process_mean_vals`,`process_sig_vals`,`process_cor_vals`: initial values for process distribution parameters
 ...
 * e.g.: `"limiting-lag-1-poly-2"` would model the covariate to have a limiting affecting recruitment the next year (lag = 1) as a second order orthogonal polynomial: $b_0 + b_1  ecov +  b_2  ecov^2$
...
# Fitting effects on recruitment
...
Diagnostics of the GSI index without effects
.pull-left[
...
.pull-left[
Check parameter estimates without GSI effects on recruitment: 
...
Now change the model to include a limiting effect (affects maximum recruitment) of the GSI
```{r, eval = FALSE}
...
Diagnostics of the GSI index with effect on recruitment
.pull-left[
```{r, echo = FALSE, out.width="95%", fig.align="center"}
...
.pull-left[
Check parameter estimates with GSI effects on recruitment: 
* effect size
...
.pull-left[
Check parameter estimates without GSI effects on M: 
...
Check diagnostics of the GSI index again with effects on M
.pull-left[
```{r, echo = FALSE, out.width="95%", fig.align="center"}
...
.pull-left[
Check parameter estimates with GSI effects on M: 
*  effect size
...
# Fitting effects on recruitment and M
...
# Fitting effects on catchability

Change input to exclude any GSI effects, but include random effects on catchability for the first index.
```{r, eval = FALSE}
catchability <- list(
  re = c("iid", "none")
)
...
.pull-left[
Check parameter estimates without GSI effects on catchability: 
With effect of GSI on catchability check: 
...
Check diagnostics of the GSI index again with effects on catchability:
.pull-left[
```{r, echo = FALSE, out.width="95%", fig.align="center"}
...
.pull-left[
Check parameter estimates with GSI effects on catchability: 
*  effect size
