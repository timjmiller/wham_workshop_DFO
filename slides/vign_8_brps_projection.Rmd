...
The `basic_info` argument to `prepare_wham_input` is a list with these elements to configure SPR-based reference points.
* `$percentSPR`: percentage (0-100) of unfished SSB/R to use for SPR-based reference points (0-100).
* `$XSPR_R_avg_yrs`: which years to average annual recruitments for SSB and Yield at SPR-based F. Default is all data years.
* `$XSPR_R_opt`: whether to use annual estimated or expected recruitments. 
    * Expected recruitment is constant over years without SR model or covariates.
* `$XSPR_input_average_years`: which years to average inputs to per recruit calculation (selectivity, M, WAA, maturity, movement) for prevailing (static) MSY- and SPR-based reference points. Default is last 5 years.
* `$average_years_M`: which years to average M for static SSB and yield at F RPs. 
* `$average_years_move`: which years to average movement rates for static SSB and yield at F RPs.
* `$average_years_mat`: which years to average maturity for static SSB and yield at SPR-based F RPs.
* `$average_years_waassb`: which years to average WAA for static SSB at SPR-based F RPs.
* `$average_years_sel`: which years to average fishery selectivity for static SSB and yield at SPR-based F RPs.
* `$average_years_waacatch`: which years to average WAA for static catch (Yield) at SPR-based F RPs.
---
...
Here we will set up alternative reference point configurations for an otherwise identical model.
...
```
The annual recruitments are used to fill out `R_XSPR`:
```{r}
...
```
Turns out, the prevailing BRP currently just uses the last value from `R_XSPR`:
```{r}
...
The second reference point configuration uses `XSPR_R_opt=2`:
* Mean annual recruitment for the last 10 years is supposed to be used for annual BRPs.
...
The third reference point configuration uses `XSPR_R_opt=3`:
* Annual expected recruitment rather than estimated recruitment is used for annual BRPs.
* Annual expected recruitment rather than estimated recruitment is used for the average.
...
Like `XSPR_R_opt=1`, since the prevailing conditions use the value in the last year for `$R_XSPR`, it is not based on the average recruitment.
```{r}
...
The fourth reference point configuration uses `XSPR_R_opt=4`:
* Mean annual expected recruitment for the last 10 years is supposed to be used for annual BRPs.
...
`proj.opts` argument to `project_wham` and `prepare_projection` for population attributes
...
* `$proj_R_opt`: (integer)
    ...
    * 3: average recruitment deviations over `$avg.yrs.R` (if `$sigma = "rec"`) 
    * 4: no recruitment deviations (if `$sigma = "rec"`)
* `$proj_NAA_opt`: (integer)
    * 1: continue any RE processes for NAA, 
    * 2: average NAA deviations over `$avg.yrs.NAA`. 
    * 3: no NAA deviations.
* `$avg.yrs` (vector) specify which years to average over for WAA, selectivity, maturity, M, etc. when calculating reference points or average F in projection years. Default = last 5 model years. Options below for setting these individually.
...
`proj.opts` argument to `project_wham` and `prepare_projection` for population attributes (continued)

* `$avg.yrs.R`: list (length = n_stocks), years to average recruitment deviations for each stock and region for projection years (if `$proj_R_opt = 3`). Also used for BRPs in projection years. Default = last 5 years.
* `$avg.yrs.NAA`: list (length = n_stocks, each is a list with length = n_regions), years to average NAA deviations for each stock and region for projection years (if `$proj_NAA_opt = 2`). Also used for BRPs in projection years. Default = last 5 years.
* `$avg.yrs.waassb`: list (length = n_stocks), years to average weight at age for each stock SSB for projection years (if `$proj_waa` is NULL). Also used for BRPs in projection years. Default = last 5 years.
* `$avg.yrs.mature`: list (length = n_stocks), years to average maturity at age for each stock for projection years (if `$proj_mature` is NULL). Also used for BRPs in projection years. Default = last 5 years.
* `$avg.yrs.M`: list (length = n_stocks, each is a list with length = n_regions), years to average natural mortality at age for each stock and region for projection years (if `$cont.M.re = FALSE`). Also used for BRPs in projection years. Default = last 5 years.
* `$avg.yrs.move`: list (length = n_stocks, each is a list with length = n_regions), years to average movement rates at age and season for each stock and region (at beginning of interval) for projection years (if `$cont.move.re = FALSE`). Also used for BRPs in projection years. Default = last 5 years, tail(model$years, 5).
...
* `$proj_waa`: user-supplied weight-at-age array (for catch and ssb) for the projection years. Overrides averaging over years.
...
`proj.opts` argument to `project_wham` and `prepare_projection` for fishing mortality or catch
...
* `$proj_F_opt`: (vector) integers specifying how to configure each year of the projection (overrides any of the above specifications): 
    * 1: use terminal F
    * 2: use average F
    * 3: use SPR-based F
    * 4: use specified F
...
`proj.opts` argument to `project_wham` and `prepare_projection` for fishing mortality or catch (continued)

* `$proj_Fcatch`: (vector or matrix), catch or F values to use each projection year: values are not used for years using Fmsy, FXSPR, terminal F or average F. Overrides any of the above specifications of `$proj.F` or `$proj.catch`. If vector, total catch or F is supplied, else matrix columns should be fleets for fleet-specific F or Catch (n.yrs x 1 or n_fleets).
...
`proj.opts` argument to `project_wham` and `prepare_projection` for covariates:
